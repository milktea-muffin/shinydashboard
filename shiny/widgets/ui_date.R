tab_widgets_date <- tabItem(tabName = "widgets_date",
  fluidRow(
    ##----------------------------------------------------------------------------##
    ## dateInput, dateRangeInput
    ##----------------------------------------------------------------------------##
    box(
      # パラメータ
      title = "Date",
      status = "primary",
      solidHeader = TRUE,
      collapsible = TRUE,
      
      # 本文
      dateInput(inputId = "widgets_date_input1", # required
                label = "Date input",  # required
                value = "2020-01-01",  # NULL (default)
                min = "2018-01-01",    # NULL (default)
                max = "2023-12-31",    # NULL (default)
                format = "yyyy-mm-dd", # yyyy-mm-dd (default)
                startview = "month",   # month (defualt), year, decade
                # ----------------------------------------------------
                # yy Year without century (12)
                # yyyy Year with century (2012)
                # mm Month number, with leading zero (01-12)
                # m Month number, without leading zero (1-12)
                # M Abbreviated month name
                # MM Full month name
                # dd Day of month with leading zero
                # d Day of month without leading zero
                # D Abbreviated weekday name
                # DD Full weekday name
                # ----------------------------------------------------
                weekstart = 0,         # 0 (Sunday) to 6 (Saturday)
                language = "en"
      ),
      br(),
      br(),
      verbatimTextOutput("widgets_date_value1")
    ),
    box(
      # パラメータ
      title = "Date range",
      status = "primary",
      solidHeader = TRUE,
      collapsible = TRUE,
      
      # 本文
      dateRangeInput(inputId = "widgets_date_input2", # required
                     label = "Date range input",  # required
                     start = NULL,
                     end = NULL,
                     min = NULL,
                     max = NULL,
                     format = "yyyy-mm-dd", # yyyy-mm-dd (default)
                     startview = "month",   # month (defualt), year, decade
                     # ----------------------------------------------------
                     # yy Year without century (12)
                     # yyyy Year with century (2012)
                     # mm Month number, with leading zero (01-12)
                     # m Month number, without leading zero (1-12)
                     # M Abbreviated month name
                     # MM Full month name
                     # dd Day of month with leading zero
                     # d Day of month without leading zero
                     # D Abbreviated weekday name
                     # DD Full weekday name
                     # ----------------------------------------------------
                     weekstart = 0,         # 0 (Sunday) to 6 (Saturday)
                     language = "en",
                     separator = " to "
      ),
      br(),
      br(),
      verbatimTextOutput("widgets_date_value2")
    )
  )
)

